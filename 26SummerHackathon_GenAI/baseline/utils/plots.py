...
class Annotator:
    def __init__(self, im, line_width=3, font_size=None, pil=True, example=None):
        self.im = Image.fromarray(im) if pil else im
        self.pil = pil
        self.lw = line_width
    ...
    def rectangle(self, box, fill=None, outline=(255,255,255), width=2):
        if self.pil:
            self.draw.rectangle(
                box,
                fill=fill,
    ...
    def box_label(self, box, label, color=(255,0,0)):
        if self.pil:
            self.draw.rectangle(
                box,
                outline=color,
...
def plot_images(images, targets, paths=None, fname="images.jpg", names=None):
    ...
    example = names[0] if names and isinstance(names, dict) and len(names) > 0 else "example"
    annotator = Annotator(mosaic, line_width=round(fs / 10), font_size=fs, pil=True, example=example)
    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
